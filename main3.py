...
def main(args, mode='offline'):
    ...
    # 读取对应数据的info文件，包含emb_dim，利用DA_stat.ipynb生成
    with open('./data/info2-2.json', 'r') as f:
        info = json.load(f)

    '''Step 1: Create item feat and user feat and feature list'''
    # user_feats = ['user_id', 'user_device', 'user_system', 'user_province', 'user_city', 'inter_count', 'refresh_mean', 'refresh_max', 'refresh_min','consumption_mean', 'consumption_max', 'consumption_min','user_age','user_gender']
    # user_feats 最后两列分别是age, gender, 读入的是list
    user_feats = ['user_id', 'user_device', 'user_system', 'user_province', 'user_city', 'inter_count', 'user_age', 'user_gender']
    # item_feats 最后一列放keywords, 读入的是list
    ...
    model = DeepFM2(args, feat_list, data_generator)
    ...
    if mode == 'online':
        model_path = './save_model/' + args.m + str(args.em_dim) + '.ckpt'
...
if __name__ == '__main__':
    ...
    setproctitle.setproctitle("deepfm Competition")
